...
import { type MessageEntry, parseMessageData, messageDataToEditableText, editableTextToMessageData, insertControlCode } from "@/lib/message-parser"
...
export function MessageEditor({ message, onUpdate }: MessageEditorProps) {
  ...
  const handleInsertControlCode = (code: number, name: string) => {
    ...
    } else if (code === 0x12 || code === 0x11) { // SFX, FADE2
    ...
    const { newText, newCursorPosition } = insertControlCode(editText, cursorPosition, code, name, args)
    setEditText(newText)
    setHasChanges(true)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-mono font-semibold text-foreground">0x{message.id.toString(16).toUpperCase().padStart(4, "0")} ({message.id})</h2>
        ...
        <div className="flex gap-2">
          <Button variant={view === "parsed" ? "default" : "outline"} size="sm" onClick={() => setView("parsed")}>
          ...
          {view === "edit" ? (
            <div className="space-y-4">
              ...
              <div className="text-xs text-muted-foreground">
                <p>Control codes are formatted as [NAME] or [NAME(0x00, 0x01)] with arguments.</p>
                <p>Use the "Insert Control Code" button to add control codes easily.</p>
              </div>
            </div>
          ) : view === "parsed" ? (
              ...
              {parsed.map((item, index) => (
                <span key={index}>
                  {item.type === "control" ? (
                      ...
                      <>
                        <Badge variant="secondary" className="bg-accent text-xs text-accent-foreground inline-flex items-center gap-1 align-middle mx-1">
                          {item.value}
                        </Badge>
                        <span className="text-muted-foreground text-xs mx-1">
                          [{item.bytes.map((b) => `0x${b.toString(16).toUpperCase().padStart(2, "0")}`).join(" ")}]
                        </span>
                      </>
                  ) : (
            ...
            <div className="font-mono text-sm">
              <div className="grid grid-cols-[auto_1fr_1fr] gap-4">
                ...
                {Array.from({ length: Math.ceil(message.data.length / 16) }).map((_, rowIndex) => {
                  const offset = rowIndex * 16
                  const rowData = message.data.slice(offset, offset + 16)

                  return (
                    <div key={rowIndex} className="contents">
                      <div className="text-primary">{offset.toString(16).toUpperCase().padStart(8, "0")}</div>
