            ...
            filteredMessages.map((msg) => (
              <Card
                ...
                className={`p-4 cursor-pointer transition-colors ${
                  selectedId === msg.id ? "bg-primary/20 border-primary" : "hover:bg-primary/50"
                }`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between">
                      <span className="text-sm font-mono font-medium text-primary">
                        0x{msg.id.toString(16).toUpperCase().padStart(4, "0")} ({msg.id})
                      </span>
                      <span className="text-xs text-muted-foreground">{msg.data.length} bytes</span>
                    </div>
                    <p className="text-sm font-mono text-foreground/80 truncate">{msg.preview}</p>
                  </div>
                </div>
              </Card>
            ))
